...
def main(num_epochs=600):
    ...
    mean = np.load("/home/heckerm/bachelor/registration/calculated_mean.npy")
    std = np.load("/home/heckerm/bachelor/registration/calculated_std.npy")
    
    train_transform = A.Compose([
    A.Rotate(p=0.24),
    A.HorizontalFlip(p=0.32),
    A.VerticalFlip(p=0.45),
    A.ColorJitter(hue=0.1, p=0.57), 
    A.Normalize(mean=list(std.round(4)), std=list(std.round(4))),
        ])
